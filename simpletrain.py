...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        writerwriter.add_scalar('LOSS', losses, global_step=epoch)
...
def train(train_list, model, criterion, optimizer, epoch):
    ...
    for i,(prev_img, img, post_img, prev_target, target, post_target ) in enumerate(train_loader):
        ...
        output = (prev_flow+post_flow)/2.0
...
def validate(val_list, model, criterion):
    ...
    writerwriter.add_scalar('MAE', mae, global_step=i)
    print(' * MAE {mae:.3f} '
              .format(mae=mae))
